...
const getPokemonDetail = async () => {
    try {
        ...
        // Evita crash caso o DOM esperado não exista (ex: elementos removidos/alterados)
...
const createPokemonDetail = (pokemon) => {
    ...
    // Limpeza para evitar duplicar conteúdo caso a página seja re-renderizada
    ...
    if (abilitiesContainer) {
        ...
        pokemon.abilities.forEach(ability => {
            const abilityName = ability?.ability?.name?.[0]
            ...
            abilityCard.addEventListener('click', async () => {
                ...
                try {
                    if (!abilityUrl) {
                        throw new Error('URL da habilidade ausente');
                    }

                    // Evita crash caso o fetch/parse falhe
                    const response = await fetch(abilityUrl);

                    if (!response.ok) throw new Error('Falha ao carregar habilidade');
                    ...
                    console.error(e);
                    abilityDescription.textContent = 'Não foi possível carregar a descrição desta habilidade.';
                    abilityDescription.classList.add('visible');
                }
            });
...
const setupHeaderControls = () => {
    ...
    const searchFromDetailToIndex = async () => {
        const query = searchInput?.value.trim();
        if (!query) {
            return;
        }

        // Novo comportamento: sempre vai direto para a página de detalhe do Pokémon.
        if (/^\d+$/.test(query)) {
            const id = Number(query);
            window.location.href = `pokemon-detail.html?id=${encodeURIComponent(String(id))}`;
            return;
        }
        ...
        try {
            ...
            const data = await response.json();
            window.location.href = `pokemon-detail.html?id=${encodeURIComponent(String(data.id))}`;
        } catch (error) {
            console.error(error);
